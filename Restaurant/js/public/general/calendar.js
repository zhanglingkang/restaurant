...
define(function (require, exports, module) {
    ...
    function getChineseCalendar(date) {
        ...
        return {
            ...
            chineseDay: getChineseDay(date),
            chineseSolarTerm: getSolarTerm(date),
            weekName: weekNameArray[date.getDay()]
        }
    }
    ...
    function getChineseDate(date) {
        ...
        if (daysCount <= (chineseMonthDays[0] - chineseBeginDay)) {
            if ((yyyy > 1901) && (getChineseDate(new Date((yyyy - 1) + "/12/31")) < 0)) {
...
//        "午(马)", "未(羊)", "申(猴)", "酉(鸡)", "戌(狗)", "亥(猪)"]
        ...
        var yyyy = date.getFullYear()
        var mm = date.getMonth() + 1
        var chineseMM = parseInt(Math.abs(getChineseDate(date)) / 100)
        if (yyyy < 100) {
            yyyy += 1900
        }
        if (chineseMM > mm) {
    ...
    function getChineseMonth(date) {
        ...
        var month = parseInt(getChineseDate(date) / 100)
        if (month < 0) {
    ...
    function getChineseDay(date) {
        var chineseDayArray = ["零",
            ...
            "廿一", "廿二", "廿三", "廿四", "廿五",
            "廿六", "廿七", "廿八", "廿九", "三十"]
        var day = (Math.abs(getChineseDate(date))) % 100
...
//    }
    module.exports = {
         ...
         * @param {number}month
         */
        getCountOfMonth: function (year, month) {
